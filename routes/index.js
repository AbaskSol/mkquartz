...
router.post('/reguser', (req, res) => {
  ...
  // console.log(hw);
  connection.query(`INSERT INTO users (userid,password,role) VALUES ('${req.body.userid}','${req.body.password}','${req.body.role}')`, (err) => {
    if (!err)
    {
      res.json({ status: "Y" });
...
router.post('/getpwdforchange',(req,res) => {
  console.log(req.body);
  connection.query(`SELECT * FROM users WHERE userid = '${req.body.userId}'`, (err, result) => {
    if (err) {
      res.json({ status: "N", pwdmatch: false })
    }
    else {
      var details = result;
      if (req.body.password == details[0].password) {
        res.json({ status: "Y", pwdmatch: true });
        console.log('The password matched..');
      }
      else {
        res.json({ status: "N", pwdmatch: false });
...
router.post('/chgpassword',(req,res)=>{
  console.log("Change pwd res: ",req.body);
  connection.query(`UPDATE users SET password = '${req.body.newpassword}' WHERE userid = '${req.body.userId}' `, (err, result) => {
    if(err) {
      res.json({ status: "N", pwdmatch: true });
    }
    else {
      res.json({ status: "Y", pwdmatch: true });
    }
